    ...
    async fetch(request, env) {
        ...
        try {
            ...
            if (path === '/api/customers' && method === 'POST') {
                ...
                return json({ id: result.meta.last_row_id, name, phone, address, area }, 201);
            }
            ...
            if (path.startsWith('/api/customers/') && method === 'PUT') {
                ...
                return json({ id, name, phone, address, area });
            }
            ...
            if (path === '/api/customers/find-by-phone' && method === 'POST') {
            ...
            if (path === '/api/products' && method === 'POST') {
                ...
                return json({ id: result.meta.last_row_id, name, purchasePrice, sellingPrice, gst, stock }, 201);
            }
            ...
            if (path === '/api/orders' && method === 'POST') {
                ...
                // Generate order ID
                const countResult = await env.DB.prepare('SELECT COUNT(*) as cnt FROM orders').first();
                const nextId = (countResult?.cnt || 0) + 1;
                const orderId = `ORD-${String(nextId).padStart(3, '0')}`;

                // Insert order
                await env.DB.prepare(
                    'INSERT INTO orders (id, customer_id, subtotal, gst_amount, total, payment_status, tracking_id, status, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)'
                ).bind(orderId, customerId, subtotal, gstAmount, total, paymentStatus, '', 'pending', createdBy).run();
                ...
                return json({ id: orderId, customerId, items, subtotal, gstAmount, total, paymentStatus, status: 'pending' }, 201);
            }
